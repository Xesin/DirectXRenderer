...
ID3D12DescriptorHeap* DescriptorAllocator::RequestNewHeap(D3D12_DESCRIPTOR_HEAP_TYPE Type)
{
	...
	ASSERT_SUCCEEDED(Renderer::device->CreateDescriptorHeap(&Desc, MY_IID_PPV_ARGS(&pHeap)));
...
D3D12_CPU_DESCRIPTOR_HANDLE DescriptorAllocator::Allocate(uint32_t Count)
{
	if (m_CurrentHeap == nullptr || m_RemainingFreeHandles < Count)
	{
		...
		if (m_DescriptorSize == 0)
			m_DescriptorSize = Renderer::device->GetDescriptorHandleIncrementSize(m_Type);
	}
...
{
	ASSERT_SUCCEEDED(Renderer::device->CreateDescriptorHeap(&m_HeapDesc, MY_IID_PPV_ARGS(m_Heap.ReleaseAndGetAddressOf())));
#ifdef RELEASE
	(void)DebugHeapName;
#else
	...
	m_DescriptorSize = Renderer::device->GetDescriptorHandleIncrementSize(m_HeapDesc.Type);
